...
const complaintSchema = z.object({
  complaintSubject: z.string().min(5, { message: "Subject must be at least 5 characters long." }),
...
export async function submitComplaint(prevState: ComplaintState, formData: FormData): Promise<ComplaintState> {
  const validatedFields = complaintSchema.safeParse({
    complaintSubject: formData.get("complaintSubject"),
  ...
  if (!validatedFields.success) {
    return {
      ...
      message: "Invalid complaint details. Please check the fields.",
    };
  }

  try {
    const aiResponse = await categorizeComplaint({ 
      complaintSubject: validatedFields.data.complaintSubject,
      complaintText: validatedFields.data.complaintText 
    });
